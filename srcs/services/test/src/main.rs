...
channel
  ...
  channel1
    ...
    .await
    .unwrap();
let args = BasicPublishArguments::new(&queue_name, "example_basic_pub_sub");
let args1 = BasicPublishArguments::new(&queue_name1, "example_basic_pub_sub");

  channel
  .basic_publish(BasicProperties::default(), "test".as_bytes().to_vec(), args)
  .await
  .unwrap();
  channel1
  .basic_publish(BasicProperties::default(), "test".as_bytes().to_vec(), args1)
  .await
  .unwrap();
