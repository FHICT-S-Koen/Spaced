import { createContext, createMemo, createSignal, Index, ParentComponent, Show, useContext } from "solid-js";
...
export const OverlayProvider: ParentComponent<{
  ...
  const [els, setEls] = createSignal<Node[]>([
    <Search /> as Node
  ])
  ...
  return (
    <OverlayContext.Provider value={[state, { toggleEditOverlay }]}>
      ...
      <Show when={state.isEdit}>
        <canvas class="absolute z-40 w-screen h-screen bg-black opacity-50" oncontextmenu={handleContextMenu} />
        <Show when={menu()}>
          <div ref={menuRef} onblur={handleBlur} class="flex flex-col absolute z-50 rounded w-16 bg-slate-200">
            <button tabindex="-1" onclick={createControl} class="">test</button>
          </div>
        </Show>
      </Show>
